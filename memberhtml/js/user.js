...
user={
        addUser:function(){ //点击添加用户           
            $('.add-bt').on('click',function(){
                comment.listAjax(urlList+'/addUser',user.addUserData(),function(data){
        ...
        searchUserList:function(){  //请求用户接口
            ...
            comment.listAjax(urlList+'/searchUser',{},function(data){
        ...
        searchUserListShow:function(result){//请求用户接口渲染到页面
            ...
            $('.updata').on('click',function(){  //打开用户编辑弹框
                ...
                $('.undataUser').unbind('click').on('click',function(){  //点击请求更新用户接口
                    comment.listAjax(urlList+'/updataUser',user.updataData(_id),function(data){
                        console.log(data)
                        if(data.status == 'success'){
                            alert('更改成功')
                            $('.updataInfo').css({
            ...
            $('.delete').on('click',function(){ //点击删除用户
                ...
                $('.alert-bt-1').on('click',function(){
                    comment.listAjax(urlList+'/deleteUser',{_id:_id},function(data){
                    console.log(data)
                    if(data.status == 'success'){
                        $('.alert').remove();
                        user.searchUserList();//请求用户列表数据
