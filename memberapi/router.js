...
router.get('/uploadImg',function(req,res){  //调用账单图片
    console.log(req.query._uid,456)
    ImgUrl.find({uid:req.query._uid},function(err,doc){
        if(err) return res.status(500).json([{"status":'fail'}]);
...
router.get('/addIntermediary', (req, res) => {  //注册合伙人
            ...
            intermediary.save(function(err,result){
                if(err) return res.status(500).json({"status":'fail'});
                User.updateOne({_id:query.uid},{isIntermediary:1},function(err,doc){  //申请合伙人中
                    if(err)  return res.status(500).json({"status":'fail'});
                    return res.status(200).json({"status":"success"});
                })
...
router.get('/agreeIntermediary',function(req,res){  //同意合伙人审核
    var _id=req.query._id;
    var _iid=req.query._iid;
    User.updateOne({_id:_id},{isIntermediary:2},function(err,doc){
        if(err)  return res.status(500).json({"status":'fail'});
        Intermediary.updateOne({_id:_iid},{invitation_status:1},function(err,result){
            if(err)  return res.status(500).json({"status":'fail'});
            return res.status(200).json({"status":"success"});
        })
    })
...
router.get('/memberCount',function(req,res){  //查询合伙伙人会员数量
   Member.find({member_code:req.query.code,member_status:1},function(err,doc){
    if(err) return res.status(500).json([{"status":'fail'}]);
    return res.status(200).json(doc);
   })
...
router.get('/addMember', (req, res) => {  //注册会员
    ...
    member.save(function(err,result){
        if(err) return res.status(500).json({"status":'fail'});
        User.updateOne({_id:query.uid},{idMember:1},function(err,doc){  //更新会员状态
            if(err)  return res.status(500).json({"status":'fail'});
            return res.status(200).json({"status":"success"});
        })
...
router.get('/agreeMember',function(req,res){  //同意会员审核
    var _id=req.query._id;
    var _mid=req.query._mid;
    var code=req.query.code;
    User.updateOne({_id:_id},{idMember:2},function(err,doc){
        if(err)  return res.status(500).json({"status":'fail'});

        Member.updateOne({_id:_mid},{member_status:1},function(err,result){
            if(err)  return res.status(500).json({"status":'fail'});
            
            Member.find({member_code:code},function(err,mdata){
                console.log(mdata,111)
                var zyj=0;
                var zhye=0;
                for(var i=0;i<mdata.length;i++){
                    zyj+=mdata[i].member_consumption_amount;
                    zhye+=mdata[i].member_consumption_ye;
                }
                Intermediary.updateOne({invitation_code:code},{invitation_member_count:mdata.length,invitation_total:zyj,invitation_income:zyj*0.5,invitation_balance:zhye.toFixed(2),invitation_refunds:(zhye*0.5/12).toFixed(2)},function(err,doc){
                    if(err)  return res.status(500).json({"status":'fail'});
                    return res.status(200).json({"status":"success"});
                })
            })
...
router.get('/noagreeMember',function(req,res){  //拒绝会员审核
    var _id=req.query._id;
